...
public class ProjectCinemaTest {
	...
	public void step1_searchMovie() throws Exception {
		...
		WebElement searchButton = wait.until(ExpectedConditions.elementToBeClickable(By.name("Search")));
		searchButton.click();
		
		WebElement searchField = wait.until(ExpectedConditions.presenceOfElementLocated(By.className("UIASearchBar")));
		searchField.sendKeys("Minions");
		
		Thread.sleep(3000);
		
		TouchAction action = new TouchAction(driver);
		action.longPress(50,110).perform();
	}
	...
	public void step4_timeLengthShowed() throws Exception {
		...
		assertEquals("Time length of movie is not proper.", "1h 31m", timeMovie.getText());
	}
